...
import { getLotsByPeriod } from "../../services/statService"; 

const LotsByPeriodChart = () => {
  ...
  const loadData = async (start, end) => {
    ...
    try {
      const stat = await getLotsByPeriod(start, end);
      setData(stat);
    } catch (err) {
      console.error("Failed to fetch lots by period:", err);
  ...
  return (
    <div style={{ width: "100%", padding: "20px", background: "#fff", borderRadius: "8px" }}>
      ...
      {!error && data.length === 0 && !loading && (
        <p>No lots were created in this period.</p>
      )}
        ...
        <div style={{ opacity: loading ? 0.5 : 1, transition: "opacity 0.3s ease" }}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis tickFormatter={(tick) => tick.substring(5)} dataKey="date" />
...
export default LotsByPeriodChart;